...
export default function AuthPage() {
  ...
  // A simple function to call the API:
  const handleForgotPassword = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      await api.post("/api/v1/auth/forgot-password", { email: resetEmail })
      toast.success("If an account exists, a reset link has been sent.")
      setIsForgotPassword(false) // flip back to login view
    } catch (error) {
      ...
      {isForgotPassword ? (
        ...
        <div className="w-full max-w-md space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="text-center mb-8">
            <h2 className="text-xl font-light tracking-widest uppercase text-zinc-900 mb-2">
              Reset Password
            </h2>
          ...
          <form onSubmit={handleForgotPassword} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full bg-zinc-900 text-white py-4 text-sm font-medium tracking-widest uppercase hover:bg-zinc-800 transition-colors"
            >
              Send Reset Link
            </button>
          </form>
          ...
          <div className="text-center mt-6">
            <button
              ...
              className="text-xs text-zinc-500 hover:text-zinc-900 tracking-widest uppercase transition-colors underline underline-offset-4"
            >
              Back to Sign In
            </button>
          </div>
        ...
        <div className="w-full max-w-md space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <Tabs defaultValue="login" className="w-full">
            <TabsList className="grid w-full grid-cols-2 mb-6 bg-zinc-100 rounded-none p-1">
              <TabsTrigger value="login" className="rounded-none uppercase tracking-widest text-xs font-bold data-[state=active]:bg-white data-[state=active]:text-zinc-900 data-[state=active]:shadow-sm transition-all">
                Sign In
              </TabsTrigger>
              <TabsTrigger value="signup" className="rounded-none uppercase tracking-widest text-xs font-bold data-[state=active]:bg-white data-[state=active]:text-zinc-900 data-[state=active]:shadow-sm transition-all">
                Create Account
              </TabsTrigger>
            </TabsList>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <div className="flex justify-end pt-1 pb-2">
                  <button
                    ...
                    className="text-[10px] text-zinc-500 hover:text-zinc-900 tracking-widest uppercase transition-colors"
                  >
                    Forgot Password?
                  </button>
                </div>
            ...
            <TabsContent value="signup">
              <form onSubmit={handleSignup} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-zinc-900 text-white py-4 text-sm font-medium tracking-widest uppercase hover:bg-zinc-800 transition-colors disabled:opacity-50 mt-4"
                >
